function nexData = read(fileName)
% nexData = read(fileName) -- read .nex file and return file data
% in nexfile structure.
%
% Note: This function was written by Plexon and downloaded from their
%       website.
%
% INPUT:
...
nexData.version = fread(fid, 1, 'int32');
comment = fread(fid, 256, '*char')';
% remove first zero and all characters after the first zero
comment(end+1) = 0; 
nexData.comment = comment(1:min(find(comment==0))-1);
nexData.freq = fread(fid, 1, 'double');
nexData.tbeg = fread(fid, 1, 'int32')./nexData.freq;
nexData.tend = fread(fid, 1, 'int32')./nexData.freq;
...
for variableIndex=1:nvar
    ...
    switch type
        case 0 % neuron
            ...
            nexData.neurons{neuronCount,1}.name = name;
            nexData.neurons{neuronCount,1}.varVersion = varVersion;
            nexData.neurons{neuronCount,1}.wireNumber = wireNumber;
            nexData.neurons{neuronCount,1}.unitNumber = unitNumber;
            nexData.neurons{neuronCount,1}.xPos = xPos;
            nexData.neurons{neuronCount,1}.yPos = yPos;
            fseek(fid, offset, 'bof');
            nexData.neurons{neuronCount,1}.timestamps = fread(fid, [n 1], 'int32')./nexData.freq;
            fseek(fid, filePosition, 'bof');
            
        case 1 % event
            eventCount = eventCount+1;
            nexData.events{eventCount,1}.name = name;
            nexData.events{eventCount,1}.varVersion = varVersion;
            fseek(fid, offset, 'bof');
            nexData.events{eventCount,1}.timestamps = fread(fid, [n 1], 'int32')./nexData.freq;
            fseek(fid, filePosition, 'bof');
        
        case 2 % interval
            intervalCount = intervalCount+1;
            nexData.intervals{intervalCount,1}.name = name;
            nexData.intervals{intervalCount,1}.varVersion = varVersion;
            fseek(fid, offset, 'bof');
            nexData.intervals{intervalCount,1}.intStarts = fread(fid, [n 1], 'int32')./nexData.freq;
            nexData.intervals{intervalCount,1}.intEnds = fread(fid, [n 1], 'int32')./nexData.freq;
            fseek(fid, filePosition, 'bof');  
        
        case 3 % waveform
            waveCount = waveCount+1;
            nexData.waves{waveCount,1}.name = name;
            nexData.waves{waveCount,1}.varVersion = varVersion;
            nexData.waves{waveCount,1}.NPointsWave = NPointsWave;
            nexData.waves{waveCount,1}.WFrequency = WFrequency;
            nexData.waves{waveCount,1}.wireNumber = wireNumber;
            nexData.waves{waveCount,1}.unitNumber = unitNumber;
            nexData.waves{waveCount,1}.ADtoMV = ADtoMV;
            nexData.waves{waveCount,1}.MVOfffset = MVOfffset;
            fseek(fid, offset, 'bof');
            nexData.waves{waveCount,1}.timestamps = fread(fid, [n 1], 'int32')./nexData.freq;
            wf = fread(fid, [NPointsWave n], 'int16');
            nexData.waves{waveCount,1}.waveforms = wf.*ADtoMV + MVOfffset;
            fseek(fid, filePosition, 'bof'); 
            
        case 4 % population vector
            popCount = popCount+1;
            nexData.popvectors{popCount,1}.name = name;
            nexData.popvectors{popCount,1}.varVersion = varVersion;
            fseek(fid, offset, 'bof');
            nexData.popvectors{popCount,1}.weights = fread(fid, [n 1], 'double');
            fseek(fid, filePosition, 'bof');
            
        case 5 % continuous variable
            contCount = contCount+1;
            nexData.contvars{contCount,1}.name = name;
            nexData.contvars{contCount,1}.varVersion = varVersion;
            nexData.contvars{contCount,1}.ADtoMV = ADtoMV;
            nexData.contvars{contCount,1}.MVOfffset = MVOfffset;
            nexData.contvars{contCount,1}.ADFrequency = WFrequency;
            fseek(fid, offset, 'bof');
            nexData.contvars{contCount,1}.timestamps = fread(fid, [n 1], 'int32')./nexData.freq;
            nexData.contvars{contCount,1}.fragmentStarts = fread(fid, [n 1], 'int32') + 1;
            nexData.contvars{contCount,1}.data = fread(fid, [NPointsWave 1], 'int16').*ADtoMV + MVOfffset;
            fseek(fid, filePosition, 'bof'); 
         
        case 6 % marker
            markerCount = markerCount+1;
            nexData.markers{markerCount,1}.name = name;
            nexData.markers{markerCount,1}.varVersion = varVersion;
            fseek(fid, offset, 'bof');
            nexData.markers{markerCount,1}.timestamps = fread(fid, [n 1], 'int32')./nexData.freq;
            for markerFieldIndex=1:NMarkers
                markerName = fread(fid, 64, '*char')';
                % remove first zero and all characters after the first zero
                markerName(end+1) = 0;
                markerName = markerName(1:min(find(markerName==0))-1);
                nexData.markers{markerCount,1}.values{markerFieldIndex,1}.name = markerName;
                for markerValueIndex = 1:n
                    markerValue = fread(fid, MarkerLength, '*char')';
                    % remove first zero and all characters after the first zero
                    markerValue(end+1) = 0;
                    markerValue = markerValue(1:min(find(markerValue==0))-1);
                    nexData.markers{markerCount,1}.values{markerFieldIndex,1}.strings{markerValueIndex, 1} = markerValue;
                end
            end
            fseek(fid, filePosition, 'bof');
